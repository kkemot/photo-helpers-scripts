...
remove_empty_dirs() {
            ...
            if rmdir "$dir" 2>/dev/null; then
                ...
                if [ -d "$dir" ]; then
                    ...
                    if [ "$media_files" -eq 0 ]; then
                        ...
                        find "$dir" -maxdepth 1 -name ".*" -type f \( -name "*.ini" -o -name "*.db" -o -name ".picasa*" -o -name ".DS_Store" -o -name "Thumbs.db" \) -delete 2>/dev/null
                        ...
                        if [ "$hidden_files" -gt 0 ]; then
                            local example_hidden=$(find "$dir" -maxdepth 1 -name ".*" -type f 2>/dev/null | head -1)
                            if [ -n "$example_hidden" ]; then
                                echo "  Contains.*hidden files (e.g. $(basename "$example_hidden"))"
                            fi
                        fi
                        if [ "$subdirs" -gt 0 ]; then
                            local example_subdir=$(find "$dir" -mindepth 1 -maxdepth 1 -type d 2>/dev/null | head -1)
                            if [ -n "$example_subdir" ]; then
                                echo "  Contains $subdirs subdirectories (e.g. $(basename "$example_subdir"))"
                            fi
                        fi
    ...
    if [ "$dry_run" = "yes" ]; then
        ...
        if [ "$with_hidden" -gt 0 ]; then
            printf "  (including.*with hidden system files)\n" "$with_hidden"
        fi
